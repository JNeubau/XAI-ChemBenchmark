...
def create_ranking_plots(rankings_df, output_dir, timestamp):
    ...
    # --- Alternative to interpolation: fill missing ranks with max rank + 1 (worst rank) ---
    # Reindex to ensure all features have all models as columns
    all_models = list(rankings_df.index.get_level_values('Model').unique())
    bump_pivot = bump_pivot.reindex(columns=all_models)
    # For each feature, fill missing ranks with (max rank for that model + 1)
    for model in bump_pivot.columns:
        max_rank = bump_pivot[model].max()
        fill_value = 30  # Use a fixed value for fill, e.g., 30, or max_rank + 1
        # fill_value = max_rank + 1 if not np.isnan(max_rank) else 1
        bump_pivot[model] = bump_pivot[model].fillna(fill_value)

    # --- Force model order: LIME, SHAP, MMACE, MEG (if present), with MEG always last ---
    model_order = list(rankings_df.index.get_level_values('Model').unique())
    if 'MEG' in model_order:
        model_order = [m for m in model_order if m != 'MEG'] + ['MEG']
    desired_order = [m for m in model_order if m in rankings_df.index.get_level_values('Model').unique()]
    ...
    # Assign a unique color to each feature using a qualitative matplotlib colormap for maximum distinction
    import matplotlib
    n_features = len(bump_pivot.index)
    if n_features <= 20:
        feature_colors = sns.color_palette('tab20', n_colors=n_features)
    else:
        base_maps = ['tab20', 'tab20b', 'tab20c']
        feature_colors = []
        for cmap_name in base_maps:
            cmap = matplotlib.cm.get_cmap(cmap_name)
            feature_colors.extend([cmap(i) for i in range(cmap.N)])
            if len(feature_colors) >= n_features:
                break
        if len(feature_colors) < n_features:
            extra = sns.color_palette('hsv', n_colors=n_features - len(feature_colors))
            feature_colors.extend(extra)
        feature_colors = feature_colors[:n_features]
    ...
    n_features_worst = len(bump_pivot_worst.index)
    if n_features_worst <= 20:
        feature_colors_worst = sns.color_palette('tab20', n_colors=n_features_worst)
    else:
        base_maps = ['tab20', 'tab20b', 'tab20c']
        feature_colors_worst = []
        for cmap_name in base_maps:
            cmap = matplotlib.cm.get_cmap(cmap_name)
            feature_colors_worst.extend([cmap(i) for i in range(cmap.N)])
            if len(feature_colors_worst) >= n_features_worst:
                break
        if len(feature_colors_worst) < n_features_worst:
            extra = sns.color_palette('hsv', n_colors=n_features_worst - len(feature_colors_worst))
            feature_colors_worst.extend(extra)
        feature_colors_worst = feature_colors_worst[:n_features_worst]
